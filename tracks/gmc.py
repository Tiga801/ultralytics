"""Global Motion Compensation (GMC) for object tracking.

This module provides methods for tracking and detecting objects based on several
tracking algorithms including ORB, SIFT, ECC, and Sparse Optical Flow.
"""
...
class GMC:
    ...
    This class provides methods for tracking and detecting objects based on several tracking
    algorithms including ORB, SIFT, ECC, and Sparse Optical Flow.

    Attributes:
        method: The tracking method to use ('orb', 'sift', 'ecc', 'sparseOptFlow', 'none').
        downscale: Factor by which to downscale the frames for processing.
        prevFrame: Previous frame for tracking.
        prevKeyPoints: Keypoints from the previous frame.
        prevDescriptors: Descriptors from the previous frame.
        initializedFirstFrame: Flag indicating if the first frame has been processed.
    """
    ...
    def __init__(self, method: str = "sparseOptFlow", downscale: int = 2) -> None:
        ...
        Args:
            method: The tracking method to use.
            downscale: Downscale factor for processing frames.
        """
        super().__init__()
    ...
    def apply(self, raw_frame: np.ndarray, detections: list | None = None) -> np.ndarray:
        ...
        Args:
            raw_frame: The raw frame to be processed, with shape (H, W, C).
            detections: List of detections to be used in the processing.
    ...
    def apply_ecc(self, raw_frame: np.ndarray) -> np.ndarray:
        ...
        c = raw_frame.shape[2] if raw_frame.ndim == 3 else 1
