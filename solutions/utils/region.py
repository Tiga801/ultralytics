...
def point_in_any_region(
    point: Tuple[float, float],
    regions: List[List[Tuple[int, int]]]
) -> bool:
    """Check if point is inside ANY of the provided regions.

    Args:
        point: (x, y) coordinates.
        regions: List of polygon regions, each a list of (x, y) vertices.

    Returns:
        True if point is in at least one region, or if no regions defined.
    """
    if not regions:
        return True

    return any(point_in_region(point, region) for region in regions)


def parse_roi_regions(areas_info: List[Dict[str, Any]]) -> List[List[Tuple[int, int]]]:
    """Parse areas_info into list of polygon regions.

    Args:
        areas_info: List of area dicts with 'points' key.

    Returns:
        List of polygons (each polygon is list of (x, y) tuples).
    """
    regions = []
    for area in areas_info or []:
